var System = function() {
	...
	this.init = function() {
		...
		this.clear();
		player.setPosition(0, 2); // the entry walkway in the mansion
				...
				if (!player.actions(command, commandData)) {
					if (!handleInventoryItems(command, commandData)) {
						if (!system.actions(command, commandData)) {
							system.println("What happened? It looks like '" + command + "' is not part of this universe.");
						}
					}
				}
			}
	...
	this.actions = function(command, commandData) {
		if (command == "debug") {
			if (commandData[0] == "inventory") {
				...
				return true;
			} else if (commandData[0] == "rabbithole") {
				// if(typeof commandData[1] != typeof undefined) {
				//	  
				// }
				var playerPos = player.getPosition();
				console.log("rabbithole data:", world.getRoom(playerPos.zone, playerPos.room));
				console.log("items in your inventory:", world.getItems(playerPos.zone, playerPos.room));
				return true;
			}
